        ...
        try {
            ...
            if ((detailContent.getId() != null)) {
                if(detailContent.getOssId() != null){
                    ...
                    // 修改oss表图片存储链接
                    objectStorageService.setId(Long.valueOf(detailContent.getOssId()));
                    ossFileMapper.updateById(objectStorageService);
                }else{
                    ossFileMapper.insert(objectStorageService);
                    detailContent.setOssId(ossFileMapper.selectOne(new QueryWrapper<ObjectStorageService>().eq("file_name",fileName)).getId());
                }
                // 修改内容
                detailContentMapper.updateById(detailContent);
                return ossFileMapper.updateById(objectStorageService)>0 && detailContentMapper.updateById(detailContent)>0;
            } else {
                ossFileMapper.insert(objectStorageService);
                detailContent.setOssId(ossFileMapper.selectOne(new QueryWrapper<ObjectStorageService>().eq("file_name",fileName)).getId());
                detailContentMapper.insert(detailContent);
                return ossFileMapper.insert(objectStorageService)>0 && detailContentMapper.insert(detailContent)>0;
            }
